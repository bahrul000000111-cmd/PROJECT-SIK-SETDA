...
const RegisterSpmPage = () => {
  ...
  const totalNominal = transactions.reduce((s, t) => s + (t.nominal || 0), 0);
  const totalPajak = transactions.filter(t => t.adaPajak).reduce((s, t) => s + (t.nominalPajak || 0), 0);
  const countByJenis = transactions.reduce((acc, t) => {
    ...
    <div className="flex flex-col gap-6 pb-10">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Daftar seluruh Surat Perintah Membayar yang telah diinput ke sistem.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-5 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Total SPM</p>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-5 shadow-sm flex items-center gap-4">
          <div className="w-12 h-12 bg-emerald-50 dark:bg-emerald-900/20 rounded-xl flex items-center justify-center shrink-0">
            <BarChart2 size={22} className="text-emerald-600 dark:text-emerald-400" />
          </div>
          <div>
            <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Total Belanja</p>
            <p className="text-lg font-bold text-gray-900 dark:text-white">{formatRupiah(totalNominal)}</p>
          </div>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-5 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Total Pajak</p>
            <p className="text-lg font-bold text-gray-900 dark:text-white">{formatRupiah(totalPajak)}</p>
          </div>
        </div>
      </div>

      {/* Ringkasan per Jenis SPM */}
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50">
          <h3 className="font-semibold text-gray-900 dark:text-white flex items-center gap-2">
            <ClipboardList size={18} className="text-blue-500" /> Daftar Register SPM
          </h3>
        </div>
          ...
          <table className="w-full text-left border-collapse text-sm whitespace-nowrap">
            <thead>
              <tr className="bg-gray-50/80 dark:bg-gray-800/80 border-b border-gray-200 dark:border-gray-700">
                {['No', 'Tanggal SPM', 'Nomor SPM', 'Jenis SPM', 'No. TBP', 'Sub Kegiatan', 'Uraian', 'Nominal', 'Jenis Pajak', 'NTPN', 'Nominal Pajak'].map(h => (
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
              {transactions.length === 0 ? (
                <tr>
                  <td colSpan="11" className="px-6 py-16 text-center text-sm text-gray-400 dark:text-gray-500">
                    <ClipboardList size={40} className="mx-auto mb-3 text-gray-300 dark:text-gray-700" />
                    Belum ada data SPM. Input transaksi melalui menu Belanja.
                  </td>
                </tr>
                ...
                <tr key={tx.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition-colors">
                  ...
                  <td className="px-4 py-3 text-gray-700 dark:text-gray-300">{tx.tanggalSpm || tx.tanggal || '-'}</td>
                  ...
                  <td className="px-4 py-3 font-mono text-gray-600 dark:text-gray-400">{tx.adaPajak ? tx.ntpn : '–'}</td>
                  <td className="px-4 py-3 text-right text-amber-700 dark:text-amber-400 font-medium">{tx.adaPajak ? formatRupiah(tx.nominalPajak) : '–'}</td>
                </tr>
              ))}
            </tbody>
            {transactions.length > 0 && (
              <tfoot>
                <tr className="bg-gray-50 dark:bg-gray-800 border-t-2 border-gray-200 dark:border-gray-700">
                  <td colSpan="7" className="px-4 py-3 text-sm font-bold text-gray-900 dark:text-white text-right">TOTAL</td>
